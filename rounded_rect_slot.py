...
from arc_slot import mill_tabbed_arc_slot
...
def create_rounded_rect_slot(xl: float, xr: float, yf: float, yb: float, zb: float, zt: float, r: float, tabs: bool = False):
    ...
    ramp_rate = int(min(config.FEED_RATE, ramp_length / config.DEPTH_OF_CUT * config.PLUNGE_RATE))

    def dz(d):
        return max(z - config.DEPTH_OF_CUT * d / ramp_length, zb)
