...
from gcode import goto_xy, mill_arc
from state import state
from basics import retract, approach, plunge
from utils.arc import preprocess_arc
from utils.loop import loop_z
from utils.main import main_call
from utils.polar import to_polar, from_polar, get_angle_delta


def mill_arc_slot(xb: float, xe: float, yb: float, ye: float, zb: float, zt: float, r: float, clockwise: bool,
                  shift: float = 0.0):

    xb, xe, yb, ye, xc, yc, r = preprocess_arc(xb, xe, yb, ye, zb, zt, r, clockwise, shift)
    xb, xe, yb, ye, clockwise = state.swap_to_closest_xy(xb, xe, yb, ye, clockwise)
